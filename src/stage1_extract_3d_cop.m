...
for t_idx = 1:length(subject_info.trial_nums)
    ...
    [map_level, roll_frames] = parse_roll_off_file(roll_path, is_right, grid_dims, frame);
    ...
    if ~isempty(max_path) && max_size >= 1000
        map_level_max = parse_max_image_file(max_path, is_right, size(map_level, 1), size(map_level, 2));
    end
...
function [cop_data, detected_side, grid_dims] = parse_cop_file(cop_path, configured_side)
    ...
    % Extract rectangle dimensions if present
...
function [map_level, num_frames] = parse_roll_off_file(roll_path, is_right, grid_dims, expected_frames)
    map_level = [];
    num_frames = 0;
    ...
    % Find all lines indicating frame headers (e.g. "Frame 0 (0.00 ms)" or "Frame 0")
    frame_header_indices = find(~cellfun(@isempty, regexpi(lines, '^Frame\s+\d+')));
    ...
    if length(frame_header_indices) >= 2
        block_span = frame_header_indices(2) - frame_header_indices(1) - 1;
        % Account for possible delimiter lines
        row_count = 0;
        for r = (frame_header_indices(1) + 1):(frame_header_indices(2) - 1)
            v = str2num(lines{r}); %#ok<ST2NM>
            if ~isempty(v)
    ...
    % Check first frame to determine width
    first_f_start = frame_header_indices(1) + 1;
    first_row_v = str2num(lines{first_f_start}); %#ok<ST2NM>
    ...
    for f_i = 1:num_to_read
        ...
        for l_idx = start_idx:length(lines)
            if curr_row > map_len, break; end
            if ~isempty(regexpi(lines{l_idx}, '^Frame\s+\d+')), break; end
            
            row_v = str2num(lines{l_idx}); %#ok<ST2NM>
            if ~isempty(row_v)
...
function [map_level_max] = parse_max_image_file(max_path, is_right, fallback_len, fallback_wid)
    map_level_max = [];
    ...
    % Find where numeric matrix begins (skip headers)
    matrix_rows = [];
    for k = 1:length(lines)
        ln = lines{k};
        vals = str2num(ln); %#ok<ST2NM>
        % Line has numeric matrix if it has >= 10 numerical entries or matches expected width
